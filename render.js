  ...
  return inlines
    .map((tok) => {
      ...
      switch (tok.type) {
        ...
        case "link":
          return `<a href=\"${escapeHtml(tok.url)}\" target=\"_blank\" rel=\"noopener noreferrer\">${renderInlines(tok.children || [])}</a>`;
        case "image":
          return `<img class=\"inline-img\" src=\"${escapeHtml(tok.url)}\" alt=\"${escapeHtml(tok.alt)}\" />`;
        default:
          return "";
      }
    })
...
function renderBlock(block) {
  ...
  switch (block.type) {
    case "heading": {
      ...
      const inner = block.inlines ? renderInlines(block.inlines) : escapeHtml(block.content || "");
    ...
    case "quote": {
      if (block.inlines) return `<blockquote>${renderInlines(block.inlines)}</blockquote>`;
      return `<blockquote>${escapeHtml(block.content || "")}</blockquote>`;
    }
    case "image": {
      return `<div class=\"img-wrap\"><img src=\"${escapeHtml(block.url)}\" alt=\"${escapeHtml(block.alt)}\"/></div>`;
    }
    case "link": {
      const inner = block.inlines ? renderInlines(block.inlines) : escapeHtml(block.text || "");
      return `<p><a href=\"${escapeHtml(block.url)}\" target=\"_blank\" rel=\"noopener noreferrer\">${inner}</a></p>`;
    }
    case "table": {
...
const htmlCards = cards
  .map((card) => {
    const blocks = (card.blocks || []).flatMap((b) => (Array.isArray(b) ? b : [b]));
